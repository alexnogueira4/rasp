...
class Device extends EventEmitter {
  ...
  _connect (callback) {
    ...
    this.client.on('error', (err) => {
      console.log('_connect Error: %s', err.message)
      this.client.close()
    })
  ...
  _tryJoin (callback) {
    console.log('_tryJoin')
    // We are already connected
    if (this.client) return callback()
    console.log('_tryJoin 2')
    
    this._connect(() => {
      console.log('_tryJoin 3')
      this._launch(null, (err, player) => {
        console.log('_tryJoin 4')
        if (err) {
          console.log('_tryJoin 5 - erro')
          this.client = null
          return callback(err)
        }
        console.log('_tryJoin 6')
        
        this._onLaunch(player)
        console.log('_tryJoin 7')
  ...
  getStatus (callback) {
    ...
    this._tryJoin((err) => {
      ...
      if (err) return callback(err)
      console.log("getStatus 4", this.player)
